const MicroModal = (() => {
  ...
  class Modal {
    ...
    async showModal (event = null) {
      ...
      return this.config.onShow(this.modal, this.activeElement, event)
    }
  ...
  // Reference to all instantiated modal objects
  let activeModals = {}
   ...
   * @return {void}
   */
  const show = async (targetModal, config) => {
    ...
    if (!activeModals[targetModal]) {
      activeModals[targetModal] = new Modal(options)
    }

    return activeModals[targetModal].showModal() // eslint-disable-line no-new
  }
